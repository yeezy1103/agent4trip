...
class MultiAgentTripPlanner:
    ...
    def _build_planner_query(
        ...
        structured_candidates = parsing_service.build_attraction_candidates_payload(
            candidate_attractions,
            weather_info,
        )
        structured_hotels = parsing_service.build_hotel_candidates_payload(
            candidate_hotels,
            candidate_attractions,
        )
...
**真实酒店候选(只允许从以下列表中选择,如列表为空则返回 hotel=null 或补充说明待确认):**
{structured_hotels}

**酒店原始搜索结果(仅供校对,以真实酒店候选为准):**
{hotels}

**要求:**
1. 每天安排2-3个景点;高风险天气日只安排1-2个景点
2. 每天必须包含早中晚三餐
3. 每天推荐一个具体的酒店,且必须从真实酒店候选中选择;如候选为空可返回hotel=null
4. 同一天优先把相近景点放在一起,控制单日活动半径,不要把两个很近的景点无理由拆到不同天
5. 同一天的景点顺序必须顺路,避免先去远处再返回原区域再回远处的明显回头路
6. 多日行程要保持片区推进连续性,避免出现“北边-南边-北边”这类跨日往返跳区
7. 酒店要兼顾当天收尾景点与次日首段景点;如次日主游区域明显切换,允许酒店位置随之迁移,不要为了同一酒店导致次日长距离折返
8. 返回完整的JSON格式数据
9. 景点的经纬度坐标要真实准确,且必须直接来自真实景点候选列表
10. 高风险天气日让餐饮、酒店尽量靠近景点,中风险天气日优先安排同一区域或顺路片区
11. weather_info字段必须以结构化天气风险为准,不要自行编造额外日期
12. overall_suggestions必须给出天气相关出行建议,并说明多日片区衔接逻辑
13. day.description只能概述当天实际写入attractions数组和hotel字段中的地点,不要提及未出现在结构化字段里的额外景点名称
14. day.description需要解释路线组织逻辑,例如“同片区顺路串联”“酒店便于衔接次日上午景点”
15. 高风险天气日若可选景点不足,优先减少景点数量并保留真实地点,不要编造新的地点、经纬度或酒店
16. attractions数组中的每个景点名称、地址、经纬度、类别必须与真实景点候选一致,不要改名或补造
17. 若候选列表中没有足够景点,宁可少安排,也不要补充候选列表之外的地点
18. hotel字段中的名称、地址、经纬度、价格范围、评分、类型必须与真实酒店候选一致,不要改名或补造
"""
