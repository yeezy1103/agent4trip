...
**示例:**
用户: "搜索广州的历史文化景点"
你的回复: [TOOL_CALL:amap_maps_text_search:keywords=历史文化,city=广州]

用户: "搜索云南的户外景点"
你的回复: [TOOL_CALL:amap_maps_text_search:keywords=户外,city=云南]
...
**示例:**
用户: "查询广州天气"
你的回复: [TOOL_CALL:amap_maps_weather:city=广州]
...
**示例:**
用户: "搜索广州的酒店"
你的回复: [TOOL_CALL:amap_maps_text_search:keywords=酒店,city=广州]
...
5. **必须优先保留并输出 location 坐标**
6. 如果工具结果中某家酒店没有可解析的 location,则不要把这家酒店放进最终 JSON 数组
7. 不要省略工具结果里已有的 longitude/latitude,不要只返回 name/address/type/id

**输出格式要求:**
当工具返回结果后，你必须只输出一个 JSON 数组（不要输出 Markdown/解释文字），数组每一项为一个酒店对象，字段如下：
- name: 酒店名称
- address: 地址（可选）
- location: { "longitude": <float>, "latitude": <float> }（必填）
- type: 酒店类型（可选）
- rating: 评分（可选）
- price_range: 价格范围（可选）
- id: POI id（可选）
...
- 你必须优先筛选那些带有完整坐标的酒店
- 最终 JSON 中每个酒店对象都必须能被程序直接读取到 location.longitude 和 location.latitude
"""
...
class MultiAgentTripPlanner:
    ...
    def _run_agent_with_cancellation(
        ...
        with self._debug_capture_agent_tools(agent, stage):
            result = agent.run(query)

        print("   [3/3] Agent 最终输出文本:")
        print(_preview_text(result))
        print(f"🧪 [{stage}] Agent 调试结束\n")
        ...
        try:
            for tool in tools:
                ...
                if original_run is None or not callable(original_run):
                    continue
                originals.append((tool, original_run))
                tool.run = make_wrapper(tool, original_run)
            yield
        finally:
            for tool, original_run in originals:
